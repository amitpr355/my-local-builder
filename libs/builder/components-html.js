...
Vvveb.Components.extend("_base", "html/video", {
  nodes: ["video"],
  name: "Video",
  html: '<video width="320" height="240" playsinline loop autoplay muted src="/builder/media/demo/sample.webm" poster="/builder/media/sample.webp"></video>',
  dragHtml:
  ...
  properties: [
     {
            ...
            // Properties will read after panel open
            init: function (node) {
            ...
            // User changes value -> correct element set + reload
            onChange: function (node, value /*, input*/) {
                const s = node.querySelector("source");
                const url = value || "";

                if (s) s.setAttribute("src", url);
                else node.setAttribute("src", url);

                // reload & try autoplay if allowed
                try {
                    node.load();
                } catch (e) {}
